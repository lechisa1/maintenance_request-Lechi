    ...
    <div class="row justify-content-center">
        <div class="col-md-8">
            <div class="card shadow-lg border-0">
                ...
                <div class="card-body">
                    <form action="{{ route('organization.sector.update', $sector) }}" method="POST" class="needs-validation" novalidate>
                        @csrf
                        @method('PUT')

                        <div class="mb-4">
                            <label class="form-label fw-semibold">Sector Name</label>
                            <input type="text" name="name" value="{{ old('name', $sector->name) }}" class="form-control @error('name') is-invalid @enderror" required>
                            @error('name')
